...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}Error：${plain} This script must be run with root privileges!\n" && exit 1
...
if [[ x"${release}" == x"centos" ]]; then
    if [[ ${os_version} -le 6 ]]; then
        echo -e "${red}Please use CentOS 7 or newer system！${plain}\n" && exit 1
    fi
elif [[ x"${release}" == x"ubuntu" ]]; then
    if [[ ${os_version} -lt 16 ]]; then
        echo -e "${red}Please use Ubuntu 16 or newer system！${plain}\n" && exit 1
    fi
elif [[ x"${release}" == x"debian" ]]; then
    if [[ ${os_version} -lt 8 ]]; then
        echo -e "${red}Please use Debian 8 or newer system！${plain}\n" && exit 1
    fi
fi
...
config_after_install() {
    echo -e "${yellow}Due to security reasons, you need to change the port and account password after the installation/update is completed.${plain}"
...
install_x-ui() {
    ...
    echo -e ""
    echo -e "x-ui How to use management script: "
    echo -e "----------------------------------------------"
    ...
    echo -e "x-ui enable       - View x-ui status"
